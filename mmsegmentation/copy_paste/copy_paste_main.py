...
def main(config):
    ...
    save_name = config['ann_file'].split("/")[-1][:-5]

    start_num = config['start_num']
    f = open(f"../../mmseg/{save_name}_{''.join(list(map(str,cls_ids)))}.txt","w")
    ...
    for file_name in tqdm(range(len(data))):
        ...
        save_instances(image, boxes, show_masks, box_classes, class_names, show_bbox=True, ax=None, file_name = file_name + start_num, cls_ids = cls_ids)
        f.write(f"{''.join(list(map(str,cls_ids)))}_{file_name}\n")
    f.close() 
