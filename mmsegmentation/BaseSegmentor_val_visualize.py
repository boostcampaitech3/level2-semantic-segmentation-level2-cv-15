...
##########################수정한 코드#########################################################
            if 'test' in img_metas[0][0]['filename']:
                return self.simple_test(imgs[0], img_metas[0], **kwargs)
            else:
                ...
                if self.cnt % 100 == 0 :   # 100장에 한번씩 출력
                    ...
                    gt = Image.open(f"/opt/ml/input/mmseg/annotations/{img_metas[0][0]['ori_filename'].split('.')[0]}.png")
                    gt = np.array(gt)
                    wandb.log({'pred':
                                wandb.Image(img_metas[0][0]['filename'],
                                            masks={
                                                "predictions": {
                                                    "mask_data": r[0],
